...
# Module to test
from quantforge.backtesting.backtest_strategy import Backtest, cli, AVAILABLE_STRATEGIES

# Dependent QuantForge modules
from quantforge.strategies.rsi_strategy import (
    RsiStrategy,
)  # Assuming this is one available strategy
...
def create_dummy_raw_data(tickers, start_date, end_date):
    ...
    for ticker in tickers:
        ticker_data = pd.DataFrame(
            {
                "open": [
                    100 + i * 0.1 + (hash(ticker) % 10) * 0.1 for i in range(len(dates))
                ],
                "high": [
                    101 + i * 0.1 + (hash(ticker) % 10) * 0.1 for i in range(len(dates))
                ],
                "low": [
                    99 + i * 0.1 + (hash(ticker) % 10) * 0.1 for i in range(len(dates))
                ],
                "close": [
                    100.5 + i * 0.1 + (hash(ticker) % 10) * 0.1
...
def mock_db_connection(tmp_path):
    ...
    dummy_df["timestamp"] = dummy_df["timestamp"].dt.strftime(
        "%Y-%m-%d %H:%M:%S"
...
class TestBacktestIntegration:
    ...
    def test_backtest_run_rsi_strategy_basic(self, mock_plot_show, mock_db_connection):
        ...
        start_date = datetime(
            2023, 3, 1, tzinfo=timezone.utc
        )  # Needs > 60 days buffer before this
        ...
        assert np.isclose(
            backtest.benchmark_data.iloc[0], 50000.0
...
class TestCli:
    ...
    def test_run_command_success(
        self, mock_plot, mock_print, mock_backtest_run, mock_db_connection
    ):
        ...
        assert "start_date" in call_kwargs and isinstance(
            call_kwargs["start_date"], datetime
        )
        assert "end_date" in call_kwargs and isinstance(
            call_kwargs["end_date"], datetime
        )
    ...
    def test_run_command_invalid_strategy(self):
        ...
        runner = CliRunner()
        result = runner.invoke(
            cli, ["run", "--strategy", "invalid_strat_name", "--tickers", "AAPL"]
        )
    ...
    def test_run_command_no_tickers(self):
        ...
        if "rsi_strategy" not in AVAILABLE_STRATEGIES:
            pytest.skip("rsi_strategy needed")
        result = runner.invoke(
            cli,
            [
                "run",
        ...
        assert (
            "Missing option '--tickers'" in result.output
            or "No valid tickers provided" in result.output
        )  # Depending on Click version / error handling
