        ...
        with patch("time.sleep") as mock_sleep:
            ...
            mock_func = Mock(
                side_effect=[
                    ...
                    Exception(
                        "Too Many Requests"
                    ),  # Second call - fail with rate limit
            ...
            assert (
                1.9 <= first_sleep <= 5.0
            ), f"First sleep value {first_sleep} not in expected range"
            assert (
                3.9 <= second_sleep <= 9.0
            ), f"Second sleep value {second_sleep} not in expected range"
    ...
    def test_backoff_calculation(self):
        ...
        with patch("random.uniform", return_value=1.0):
            with patch("time.sleep") as mock_sleep:
                ...
                decorated_func = retry_on_rate_limit(max_retries=3, base_delay=2)(
                    mock_func
                )
