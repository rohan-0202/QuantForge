...
def test_db(tmp_path_factory):
    ...
    cursor.executemany(
        "INSERT INTO historical_prices VALUES (?, ?, ?, ?, ?, ?, ?, ?)", historical_data
    )
    ...
    cursor.executemany(
        "INSERT INTO options_data (ticker, expiration_date, option_type, strike, last_price, bid, ask, volume, open_interest, implied_volatility, last_updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        options_data,
    )
...
class TestGetHistoricalData:
    ...
    def test_get_historical_data_days_parameter(self, test_db):
        ...
        assert len(df_ms) == 1
        assert (
            df_ms.index[0].date() >= (datetime.now() - timedelta(days=days_ms)).date()
        )
    ...
    def test_get_historical_data_invalid_ticker(self, test_db):
        ...
        with pytest.raises(
            ValueError, match=f"No historical data found for {invalid_ticker}"
        ):
...
class TestGetOptionsData:
    ...
    def test_get_options_data_invalid_ticker(self, test_db):
        ...
        invalid_ticker = "INVALID_TICKER_XYZ"
        with pytest.raises(
            ValueError, match=f"No options data found for {invalid_ticker}"
        ):
