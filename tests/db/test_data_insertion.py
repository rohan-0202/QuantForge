...
class TestDataInsertion:
    ...
    def test_save_ticker_info(self, test_db):
        ...
        with patch('src.db.data_insertion.yf.Ticker') as mock_ticker:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT company_name, sector, industry, market_cap, currency FROM ticker_info WHERE ticker = 'AAPL'")
    ...
    def test_save_historical_data_new_ticker(self, test_db):
        ...
        with patch('src.db.data_insertion.yf.Ticker') as mock_ticker:
            with patch('src.db.data_insertion.get_earliest_data_date_from_history') as mock_get_earliest:
                ...
                date_index = pd.date_range(start='2010-01-01', end='2023-01-01', freq='D')
                ...
                conn = sqlite3.connect(test_db)
                cursor = conn.cursor()
                cursor.execute("SELECT COUNT(*) FROM historical_prices WHERE ticker = 'AAPL'")
                count = cursor.fetchone()[0]
                cursor.execute("SELECT MIN(timestamp), MAX(timestamp) FROM historical_prices WHERE ticker = 'AAPL'")
    ...
    def test_save_financial_metrics(self, test_db):
        ...
        with patch('src.db.data_insertion.yf.Ticker') as mock_ticker:
            ...
            q_dates = pd.DatetimeIndex(['2022-03-31', '2022-06-30', '2022-09-30', '2022-12-31'])
            ...
            conn = sqlite3.connect(test_db)
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM financial_metrics WHERE ticker = 'AAPL' AND is_quarterly = 0")
            annual_count = cursor.fetchone()[0]
            
            # Verify quarterly data was saved correctly
            cursor.execute("SELECT COUNT(*) FROM financial_metrics WHERE ticker = 'AAPL' AND is_quarterly = 1")
    ...
    def test_save_options_data(self, test_db):
        ...
        with patch('src.db.data_insertion.yf.Ticker') as mock_ticker:
            ...
            def get_mock_option_chain(date):
                ...
                mock_chain = type('OptionChain', (), {'calls': mock_calls, 'puts': mock_puts})
                return mock_chain
            ...
            cursor.execute("SELECT COUNT(*) FROM options_data WHERE ticker = 'AAPL' AND option_type = 'call'")
            call_count = cursor.fetchone()[0]
            
            cursor.execute("SELECT COUNT(*) FROM options_data WHERE ticker = 'AAPL' AND option_type = 'put'")
