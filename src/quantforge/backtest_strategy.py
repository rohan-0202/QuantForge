...
try:
    from quantforge.strategies.abstract_strategy import SimpleTickerDataStrategy # Example, assuming it's usable
...
def get_strategy_class(name: str) -> Type[AbstractStrategy]:
    if name.lower() not in AVAILABLE_STRATEGIES:
        raise ValueError(f"Strategy '{name}' not found. Available: {', '.join(AVAILABLE_STRATEGIES.keys())}")
...
class Backtest:
    ...
    def __init__(
        ...
        self.tradeable_items_map: Dict[str, TradeableItem] = {} # Map ticker string to TradeableItem
        ...
        self.current_market_prices: Dict[TradeableItem, float] = {} # Store current prices here
    ...
    def _fetch_raw_data(
        ...
        """
        print(f"Attempting to fetch data for {tickers} from {db_name} between {start_buffer_date} and {end_date}")
        ...
        try:
            ...
            """
            params = tickers + [start_buffer_date.strftime('%Y-%m-%d %H:%M:%S'), end_date.strftime('%Y-%m-%d %H:%M:%S')]

            # Read data into pandas DataFrame
            df = pd.read_sql_query(query, conn, params=params)

            if df.empty:
                print("Warning: No data fetched from database for the given tickers and date range.")
                return pd.DataFrame()
            ...
            return df
             ...
             print("Please ensure the database exists and the 'historical_prices' table has the correct schema:")
             print("  ticker TEXT, timestamp DATETIME, open REAL, high REAL, low REAL, close REAL, volume INTEGER")
    ...
    def run(
        ...
        self.tradeable_items_map = {
            ticker: TradeableItem(id=ticker, asset_class=AssetClass.EQUITY) # Assuming EQUITY
        ...
        # Initialize strategy
        self.strategy = self.strategy_class(portfolio=self.portfolio, **self.strategy_params)
        ...
        self.raw_data = self._fetch_raw_data(tickers, start_buffer_date, end_date, db_name)
        ...
        if benchmark_ticker and benchmark_ticker in self.raw_data.index.get_level_values(TICKER).unique():
            ...
            if not bench_df.empty:
                ...
                self.benchmark_data = (bench_df / bench_df.iloc[0]) * self.initial_capital
            else:
                self.benchmark_data = pd.Series(dtype=float)
        else:
            self.benchmark_data = pd.Series(dtype=float)
            if benchmark_ticker:
                 print(f"Warning: Benchmark ticker '{benchmark_ticker}' not found in fetched data.")
        ...
        backtest_timestamps = sorted(all_timestamps[(all_timestamps >= start_date) & (all_timestamps <= end_date)])

        if not backtest_timestamps:
             print(f"No trading days found between {start_date.date()} and {end_date.date()} in the fetched data.")
             return {"error": "No trading days found for backtest period."}

        print(f"Running backtest simulation over {len(backtest_timestamps)} trading days...")
        self.equity_history = [] # Reset equity history

        # Record initial portfolio value
        self.equity_history.append((backtest_timestamps[0] - timedelta(microseconds=1), self.initial_capital)) # Record just before start
        ...
        for i, current_ts in enumerate(backtest_timestamps):
            ...
            current_value_for_print = self.portfolio.portfolio_value(self.current_market_prices) 
            if i % 100 == 0 or i == len(backtest_timestamps) - 1:
                print(f"Processing Day {i+1}/{len(backtest_timestamps)}: {current_ts.date()} - Portfolio Value: ${current_value_for_print:,.2f}")
            ...
            current_data_slice = self.raw_data[self.raw_data.index.get_level_values(TIMESTAMP) <= current_ts]
            ...
            for ticker_str, item_obj in self.tradeable_items_map.items():
                if ticker_str in current_data_slice.index.get_level_values(TICKER).unique():
            ...
            try:
                ...
                self.current_market_prices = current_prices_dict 
                 ...
                 print(f"Warning: No data found for current timestamp {current_ts.date()}")
            except Exception as e:
            ...
            try:
                 ...
                 print(f"!!! Error during strategy execution on {current_ts.date()}: {e}")
                 import traceback
            ...
            current_total_value = self.portfolio.portfolio_value(self.current_market_prices)
    ...
    def get_performance_metrics(self) -> Dict:
        ...
        metrics['final_portfolio_value'] = self.equity_history[-1][1] if self.equity_history else self.initial_capital
        ...
        # Simple Total Return
        total_return = (metrics['final_portfolio_value'] / metrics['initial_capital']) - 1
        ...
        metrics['num_trades'] = len(self.portfolio.get_closed_positions()) if hasattr(self.portfolio, 'get_closed_positions') else 0
    ...
    def plot_results(self, figsize=(12, 8), show_benchmark=True):
        ...
        try:
            ...
            plt.plot(equity_series.index, equity_series.values, label=f"{self.strategy.name} Strategy")
            ...
            if show_benchmark and self.benchmark_data is not None and not self.benchmark_data.empty:
                try:
                    ...
                    aligned_benchmark = self.benchmark_data.reindex(equity_series.index, method='ffill')
                    # Ensure we only plot where we have strategy data
                    aligned_benchmark = aligned_benchmark.loc[equity_series.index]
                    plt.plot(aligned_benchmark.index, aligned_benchmark.values, label="Buy & Hold Benchmark", linestyle="--")
                except Exception as e:
    ...
    def get_results_as_string(self) -> Optional[str]:
        ...
        if hasattr(self.strategy, 'rsi_window'): # Example for RsiStrategy
             ...
             output_lines.append(f"  oversold_threshold: {self.strategy.oversold_threshold}")
             output_lines.append(f"  overbought_threshold: {self.strategy.overbought_threshold}")
        # Add more parameter reporting as needed

        output_lines.append("\nFINAL HOLDINGS:")
        # Use portfolio methods/properties if available
        open_positions = self.portfolio.get_all_open_positions() # Assuming this method exists or can be added
        if not open_positions:
             ...
             for position in open_positions:
                 ...
                 output_lines.append(f"  {item.id}: {qty} shares @ ${current_price:,.2f} (Value: ${value:,.2f})")
...
@click.option(
    "--strategy",
    required=True,
    type=click.Choice(list(AVAILABLE_STRATEGIES.keys()), case_sensitive=False), # Use discovered strategies
...
@click.option(
    "--db-name", default="stock_data.db", show_default=True, help="Database for historical data"
)
@click.option(
    "--months", type=int, default=12, show_default=True, help="Number of months to backtest"
)
@click.option(
    "--start-cash", type=float, default=10000.0, show_default=True, help="Initial capital"
)
...
@click.option("--rsi-window", type=int, default=14, show_default=True, help="RSI window (for rsi_strategy)")
@click.option("--rsi-oversold", type=float, default=30.0, show_default=True, help="RSI oversold threshold (for rsi_strategy)")
@click.option("--rsi-overbought", type=float, default=70.0, show_default=True, help="RSI overbought threshold (for rsi_strategy)")
def run(strategy, tickers, db_name, months, start_cash, plot, benchmark, rsi_window, rsi_oversold, rsi_overbought):
    ...
    if benchmark_ticker not in ticker_list:
         print(f"Warning: Benchmark ticker '{benchmark_ticker}' is not in the list of tested tickers. Using '{ticker_list[0]}' instead.")
